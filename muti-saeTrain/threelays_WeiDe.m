...
endNeuralNum = 15;     %隐含单层神经元的数目,终止数 （400）
...
%             end
            ...
            saveas(gcf,strcat(filename, '/',num2str(m),'—',num2str(z),'wd迭代误差值'),'fig');
...
% %         tm =  startNeuralNum + (column-1) * AddNerualNum;
        ...
        title(strcat('第',num2str(m),'-',num2str(z),'，wd误差函数的变化情况'));
        saveas(gcf,strcat(filename,'/第',num2str(m),'—',num2str(z),'，wd误差函数的变化情况'),'fig') ;
        close(figure(gcf));   
        disp('*****');       
